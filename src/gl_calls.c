...
void init_uniforms(user_data_t* user_data)
{
	...
	user_data->rot_speed_loc = glGetUniformLocation(user_data->shader_program, "rot_speed");
	check_error(user_data->rot_speed_loc >= 0, "Failed to obtain uniform location for rot_speed.");
	gl_check_error("glGetUniformLocation [rot_speed]");
...
void init_vertex_data(user_data_t* user_data)
{
	...
	obj_vertex_entry_t* vertices = malloc(vertex_count * sizeof(obj_vertex_entry_t));
	check_error(vertices != NULL, "Failed to allocate memory for vertices.");

	obj_normal_entry_t* normals = malloc(normal_count * sizeof(obj_normal_entry_t));
	check_error(normals != NULL, "Failed to allocate memory for normals.");

	obj_tex_coords_entry_t* tex_coords = malloc(tex_coords_count * sizeof(obj_tex_coords_entry_t));
	...
	vertex_data_t* vertex_data = malloc(user_data->vertex_data_count * sizeof(vertex_data_t));
	...
	while ((entry_type = obj_get_next_entry(obj, &entry)) != OBJ_ENTRY_TYPE_END)
	{
		...
		case OBJ_ENTRY_TYPE_FACE:
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				new_vertex_data->color[0] = 0x88;
				new_vertex_data->color[1] = 0x88;
				new_vertex_data->color[2] = 0x88;
			}
...
void init_model(user_data_t* user_data)
{
	...
	user_data->angle_y = 0;
	user_data->rot_speed = 1.0;

}


// Function called when a key was activated
void key_Callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if( key == GLFW_KEY_RIGHT && action == GLFW_PRESS)
	{
		*rot_speed_ptr = max_f(10.0, *rot_speed_ptr + 1.0);
		printf("GLFW_KEY_RIGHT was pressed! | rot_speed is: %f\n", *rot_speed_ptr);
	}
	else if(key == GLFW_KEY_LEFT && action == GLFW_PRESS)
	{
		*rot_speed_ptr = min_f(*rot_speed_ptr - 1.0, -10.0);
		printf("GLFW_KEY_LEFT was pressed! | rot_speed is: %f\n", *rot_speed_ptr);
	}
}
...
void init_gl(GLFWwindow* window)
{
	...
	rot_speed_ptr = &user_data->rot_speed;
...
void update_gl(GLFWwindow* window)
{
	...
	glUniform1f(user_data->rot_speed_loc, user_data->rot_speed);
	gl_check_error("glUniform1f [rot_speed]");
